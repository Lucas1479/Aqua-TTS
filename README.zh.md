...
*在 NVIDIA GeForce RTX 4070 Ti SUPER (16 GB)、float16、相同模型权重（xxx-e15.ckpt + xxx_e2_s174_l32.pth）下测量。T2S 以 500 token 为目标测量；TTFP 指 first playable audio chunk，使用 `chunk_size_seconds=0.25`，在 3 个测试文本上各 5 次重复取中位数。完整方法和消融结果见 [benchmarks/README.md](benchmarks/README.md)。*
